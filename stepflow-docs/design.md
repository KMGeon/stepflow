...
DDL은 `@stepflow/infrastructure/schema.sql` 로 동봉. 마이그레이션 툴 의존성 없음 (사용자가 직접 적용).
...
- `MySqlJobRepository` — 프로덕션 (mysql2 prepared statement 직접 사용). `@stepflow/infrastructure` 패키지
- `InMemoryJobRepository` — 테스트/로컬 (DB 인프라 불필요). `@stepflow/core` 내장 기본 repository
...
- 코어 런타임 의존성 최소화 — `@stepflow/core`의 엔진/빌더/전이/해시는 순수 TS이며 런타임 의존성이 **없다**
...
- `schema.sql` DDL은 `@stepflow/infrastructure/schema.sql` 에 위치한다.

## 12. 테스트 전략 (InMemory 중심)

- **순수 단위 (브라우저 X)**: 빌더 그래프·선형/분기 전이·정적 검증·`job_key` 해시·restart 경로 복원 로직 — vitest
- **엔진 통합 (InMemory)**: `runJob` 전체 흐름·exit status 분기·실패→restart 라운드트립을 `InMemoryJobRepository` 로 검증 (DB 인프라 불필요, 빠르고 결정적). Puppeteer Page는 `@stepflow/test`의 `createFakePage` 테스트 더블로 대체
- **repository contract 테스트**: `@stepflow/test`의 `describeJobRepositoryContract` 동일 스위트를 `InMemoryJobRepository`(항상)와 `MySqlJobRepository`(`MYSQL_URL` 있으면 opt-in)에 돌려 인터페이스 적합성 보장
- **e2e 레퍼런스**: `@stepflow/samples`의 `orders_sync`로, 중간 실패 후 재시작 시 성공 step을 건너뛰고 실패 지점부터 재개됨을 검증
...
- L3 스키마 + `@stepflow/infrastructure/schema.sql`
...
| **later**       | 병렬/파티셔닝(멀티 브라우저) · Postgres 어댑터 · `@stepflow/nestjs` 어댑터                                                   |
...
- `npm install @stepflow/core` 시 무거운 전이 의존성 없이 설치됨 (puppeteer/mysql2는 peer)
