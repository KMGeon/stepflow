...
**주입된 Puppeteer `page` 위에서 선언적 Step/Flow를 실행하고, Spring Batch 충실 메타데이터(JobInstance / JobExecution / ExecutionContext)를 영속화하며, 파라미터 인스턴스 단위로 실패 지점부터 재시작하는 브라우저 RPA 배치 런타임.**
(Spring Batch의 Job / Step / JobRepository / JobInstance 개념을 브라우저 RPA 도메인으로 옮긴 오픈소스 라이브러리)

## 2. 배경 / 동기

Puppeteer 기반 RPA 자동화는 흔히 다음과 같은 구조적 요구를 공유한다:

- **Job** = 하나의 RPA 시나리오(로그인 → 조회 → 파싱 → 처리 같은 일련의 흐름)
- **Step** = 시나리오를 구성하는 단위 작업. 보통 `{ seqNo, name, desc }` 류의 메타데이터와 실제 실행 로직이 따로 관리된다
- 실행 이력(언제 무엇이 성공/실패했는지)·성공률·이상탐지를 위한 관측 레이어

이런 구조를 직접 손으로 짜다 보면 메타데이터·실행이력·실패 step 기록·관측까지는 갖추게 되지만, 다음은 매번 빠진다:
**재시도(retry)·체크포인트 재시작·chunk 처리·item 리스너·트랜잭션·파티셔닝.**

stepflow는 이 패턴을 일반화한 범용 라이브러리다:

- Step 시퀀스를 **데이터가 아닌 함수형 단위 + 조건 분기 Flow**로 표준화
- 실행이력/재시작/관측성을 **JobRepository** 로 표준화

즉 Puppeteer RPA가 반복적으로 필요로 하는 Job/Step 구조, 메타데이터, 실행이력, 실패 지점 재시작을 한곳에 담아 재사용 가능한 형태로 제공하는 것이 목표다.
...
- `puppeteer`는 **peerDependency**(타입 의존만). 직렬 큐·스텔스·프로필·브라우저 수명은 전부 소비자(브라우저를 소유하는 쪽 — 예: 스텔스 + 직렬 큐 브라우저 매니저) 소관.
...
| 개념                 | 설명                                                                    | 예시                |
| -------------------- | ----------------------------------------------------------------------- | ------------------- |
| **Job**              | 이름이 붙은 Step 흐름(Flow) 정의                                        | `orders_sync`       |
| **JobParameters**    | 실행 파라미터(날짜·스토어ID 등). 식별 파라미터는 인스턴스 정체성에 포함 | `{ date, storeId }` |
| **JobInstance**      | `(jobName + 식별 params 해시)` 로 식별되는 논리적 실행 단위             | 인스턴스            |
| **Step**             | `{ name, run(ctx) }` 함수형 단위                                        | `login` / `parse`   |
| **Flow**             | Step 간 조건 전이(transition) 그래프 (선형 기본 + 분기 예외)            | 선형 + `.branch`    |
| **JobExecution**     | JobInstance의 1회 실행(상태/시각/에러/결과)                             | 실행 row            |
| **StepExecution**    | Step 1회 실행 기록(상태/exit status/카운트)                             | step 실행 row       |
| **ExecutionContext** | Job/Step 레벨의 영속 key-value (restart 재개 상태)                      | `shared`            |
| **JobRepository**    | 위 메타데이터를 저장·조회 (MySQL / InMemory)                            | 영속 계층           |
...
- `run`이 `'FAILED'`를 반환하면 throw와 동일하게 **실제 실패**로 취급된다 (restart 기준이 됨)
...
- 소비자(브라우저를 소유하는 쪽)가 **영속 Chrome 프로필**을 쓰면 세션이 유지되어 `login` step도 건너뛸 수 있음
...
DDL은 `@kmgeon/stepflow-infrastructure/schema.sql` 로 동봉. 마이그레이션 툴 의존성 없음 (사용자가 직접 적용).
...
> 위 시그니처/스키마 스니펫은 설명용이며, **정본(canonical)은 `@kmgeon/stepflow-core`가 export하는 타입과 `@kmgeon/stepflow-infrastructure/schema.sql`** 이다.

**v0.1 구현 2종**:

- `MySqlJobRepository` — 프로덕션 (mysql2 prepared statement 직접 사용). `@kmgeon/stepflow-infrastructure` 패키지
- `InMemoryJobRepository` — 테스트/로컬 (DB 인프라 불필요). `@kmgeon/stepflow-core` 내장 기본 repository
...
| 패키지 매니저 | **`npm` workspaces**             | 모노레포(멀티 패키지)                |
| 린트/포맷     | eslint + prettier                |                                      |

### 의존성 철학

- 코어 런타임 의존성 최소화 — `@kmgeon/stepflow-core`의 엔진/빌더/전이/해시는 순수 TS이며 런타임 의존성이 **없다**
- `puppeteer`(core)/`mysql2`(infrastructure)는 **peerDependency** — stepflow가 브라우저도 커넥션도 소유하지 않고 주입받음
- ORM(Prisma/TypeORM) 미사용 — `mysql2` prepared statement 직접 사용
- 로거는 인터페이스만 의존

### 의도적 배제 (v0.1)

- ❌ NestJS 코어 결합 (후속 `@kmgeon/stepflow-nestjs` 어댑터 여지만 남김)
- ❌ Postgres 어댑터 (인터페이스만 분리)
- ❌ ORM / DI 컨테이너 / 마이그레이션 툴
- ❌ retry / chunk / 병렬 (로드맵 §15)

## 11. 패키징 (npm workspaces 모노레포)

stepflow는 단일 npm 패키지가 아니라 **npm-workspaces 모노레포**로 구성된다.

| 패키지                                | 배포    | 내용                                                                                                                                        | 의존성                                         |
| ------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **`@kmgeon/stepflow-core`**           | PUBLISH | types, errors, job-key 해시, `defineJob`, `runJob`, JobRepository 인터페이스, 메타데이터 모델, **InMemoryJobRepository**(DB-free 기본 repo) | 런타임 의존성 없음. `puppeteer`는 peer(타입만) |
| **`@kmgeon/stepflow-infrastructure`** | PUBLISH | `MySqlJobRepository` + `schema.sql`                                                                                                         | `@kmgeon/stepflow-core` 의존. `mysql2`는 peer  |
| **`@kmgeon/stepflow-test`**           | PUBLISH | `describeJobRepositoryContract` 공용 contract 스위트 + `createFakePage` Puppeteer Page 테스트 더블                                          | `@kmgeon/stepflow-core`                        |
| **`@kmgeon/stepflow-samples`**        | private | 범용 `orders_sync` 예제                                                                                                                     | core (+ test·puppeteer dev)                    |
| **`@kmgeon/stepflow-docs`**           | private | 이 설계 문서 + TypeDoc API 생성                                                                                                             | —                                              |

- 두 repository 구현(`InMemoryJobRepository` / `MySqlJobRepository`)은 `@kmgeon/stepflow-test`의 `describeJobRepositoryContract` 동일 스위트를 통과해야 한다.
- `schema.sql` DDL은 `@kmgeon/stepflow-infrastructure/schema.sql` 에 위치한다.

## 12. 테스트 전략 (InMemory 중심)

- **순수 단위 (브라우저 X)**: 빌더 그래프·선형/분기 전이·정적 검증·`job_key` 해시·restart 경로 복원 로직 — vitest
- **엔진 통합 (InMemory)**: `runJob` 전체 흐름·exit status 분기·실패→restart 라운드트립을 `InMemoryJobRepository` 로 검증 (DB 인프라 불필요, 빠르고 결정적). Puppeteer Page는 `@kmgeon/stepflow-test`의 `createFakePage` 테스트 더블로 대체
- **repository contract 테스트**: `@kmgeon/stepflow-test`의 `describeJobRepositoryContract` 동일 스위트를 `InMemoryJobRepository`(항상)와 `MySqlJobRepository`(`MYSQL_URL` 있으면 opt-in)에 돌려 인터페이스 적합성 보장
- **e2e 레퍼런스**: `@kmgeon/stepflow-samples`의 `orders_sync`로, 중간 실패 후 재시작 시 성공 step을 건너뛰고 실패 지점부터 재개됨을 검증
...
- L3 스키마 + `@kmgeon/stepflow-infrastructure/schema.sql`
...
- 대표적인 Puppeteer Job 1개(예: `orders_sync`)를 stepflow로 구현하여 선언적 Step/Flow + 메타데이터 영속이 동일 동작
...
- `npm install @kmgeon/stepflow-core` 시 무거운 전이 의존성 없이 설치됨 (puppeteer/mysql2는 peer)
