...
DDL은 `@kmgeon/stepflow/schema.sql` 로 동봉. 마이그레이션 툴 의존성 없음 (사용자가 직접 적용).
...
> 위 시그니처/스키마 스니펫은 설명용이며, **정본(canonical)은 `@kmgeon/stepflow`가 export하는 타입과 `@kmgeon/stepflow/schema.sql`** 이다.

**v0.1 구현 2종**:

- `MySqlJobRepository` — 프로덕션 (mysql2 prepared statement 직접 사용). `@kmgeon/stepflow/infrastructure` 패키지
- `InMemoryJobRepository` — 테스트/로컬 (DB 인프라 불필요). `@kmgeon/stepflow` 내장 기본 repository
...
- 코어 런타임 의존성 최소화 — `@kmgeon/stepflow`의 엔진/빌더/전이/해시는 순수 TS이며 런타임 의존성이 **없다**
...
> v0.2 설계 당시에는 npm-workspaces 모노레포(다중 패키지)였으나, 이후 **단일 패키지
> `@kmgeon/stepflow` + subpath exports**로 통합했다. 결정 근거와 운영 기준은
> [`packaging.md`](./packaging.md)를 본다. (모듈 경계 자체는 동일하다 — core /
> `./puppeteer` / `./infrastructure` / `./integration` / `./test`.)

설계상 변하지 않는 원칙:

- core(`@kmgeon/stepflow`)는 `puppeteer`/`mysql2`/`better-sqlite3` 코드를 끌어오지
  않는다 — 무거운 백엔드는 `./puppeteer` / `./infrastructure` 모듈에만 둔다.
- 두 repository 구현(`InMemoryJobRepository` / `MySqlJobRepository`)은
  `@kmgeon/stepflow/test`의 `describeJobRepositoryContract` 동일 스위트를 통과해야
  한다.
- `schema.sql` DDL은 `@kmgeon/stepflow/schema.sql` 에 위치한다.

## 12. 테스트 전략 (InMemory 중심)

> 운영 기준(실행 방법·위치 규약·새 테스트 작성)은 [`testing.md`](./testing.md). 아래는
> 설계상 테스트 레이어 정의다.
...
- **e2e 레퍼런스**: `examples/`의 `orders_sync`로, 중간 실패 후 재시작 시 성공 step을 건너뛰고 실패 지점부터 재개됨을 검증
...
- L3 스키마 + `@kmgeon/stepflow/schema.sql`
...
- `npm install @kmgeon/stepflow` 시 무거운 전이 의존성 없이 설치됨 (puppeteer/mysql2는 peer)
